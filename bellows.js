...
(function (definition) {
    ...
    Module.prototype.open = function (type) {
        ...
        this.$body.eq(this.currentIndex).addClass(this.opt.openedClass);

        if (this.isJsAnimation()) {
            this.$body.eq(this.currentIndex).slideDown(this.opt.duration, function () {
                // run callback
    ...
    Module.prototype.close = function (type) {
        ...
        this.$body.eq(this.currentIndex).removeClass(this.opt.openedClass);

        if (this.isJsAnimation()) {
            this.$body.eq(this.currentIndex).slideUp(this.opt.duration, function () {
                // run callback
                if (type === 'end' && typeof self.opt.onAnimateEnd === 'function') self.opt.onAnimateEnd();
            });
        }
    ...
    Module.prototype.closeAll = function (type) {
        ...
        this.$body.removeClass(this.opt.openedClass).slideUp(this.opt.duration, function () {
            // run callback
            if (type === 'end' && typeof self.opt.onAnimateEnd === 'function') self.opt.onAnimateEnd();
        });
